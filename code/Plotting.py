...
class Plotter:
    ...
    def prepare_files(self):
        ...
        #self.save_pca_label_fn = f'{self.plt_job_dir}/{self.network_prefix}.pca_label.pdf'
    ...
    def plot_aux_hist(self, save_fn, sim_values, title, pred_values=None, ncol_plot=3, color='blue'):
        ...
        for i_row, ax_row in enumerate(axes):
            for j_col,ax in enumerate(ax_row):
                ...
                # if self.pred_aux_loaded and p in pred_stat:
                #     x_data = pred_stat[p][0]
                #     q_true = np.sum(x < x_data) / len(x)
                #     y_data = kde(x_data)
                #     ax.vlines(x_data, 0, y_data, color='red')
                #     aux_pred_val_str = "X={:.2f}".format(x_data)
                #     aux_pred_quant_str = f'Q={int(q_true*100)}%'
                #     ax.annotate(aux_pred_val_str, xy=(x_pos, y_pos-3*dy_pos), xycoords='axes fraction', fontsize=10, horizontalalignment=ha, verticalalignment='top', color='red')
                #     ax.annotate(aux_pred_quant_str, xy=(x_pos, y_pos-4*dy_pos), xycoords='axes fraction', fontsize=10, horizontalalignment=ha, verticalalignment='top', color='red')
    ...
    def plot_pca(self, save_fn, sim_stat, pred_stat=None, num_comp=4, f_show=1.0):
    ...
    def plot_pred_est_CI(self, save_fn, pred_label, ylog=True):
