...
def make_sirm_states(num_location):
    ...
    # S: Susceptible, I: Infected, R: Recovered, A: Acquired
    compartments = 'SIRA'
...
def make_events_r(states, rates):
    ...
    for x in states_I:
        ...
        idx = {'i':i}
        e = Event( idx=idx, r=r, n='r_R_{i}'.format(i=i), g='Recover', x=xml_str )
        events.append(e)
...
def make_events_s(states, rates):
    ...
    for x in states_I:
        ...
        idx = {'i':i}
        e = Event( idx=idx, r=r, n='r_S_{i}'.format(i=i), g='Sample', x=xml_str )
        events.append(e)
...
def make_events_m(states, rates):
    ...
    # for each state pair containing 'I'
    states_I = [ x for x in states.int2lbl if 'I' in x ]
    ...
    for x,y in state_pairs:
        if x != y:
            ...
            idx = {'i':i, 'j':j}
            e = Event( idx=idx, r=r, n='r_M_{i}_{j}'.format(i=i, j=j), g='Migrate', x=xml_str )
            events.append(e)
...
def make_sirm_rates( model_variant, num_locations ):
    ...
    if model_variant is 'free_rates':
        ...
        rates = {} # build model out of inputted self.feature_set ... e.g. make_feature_rates(self.feature_set)
