...
class CnnTrainer(Trainer):
    ...
    def load_input(self):
        ...
        if self.tensor_format == 'csv':
            ...
            self.stat_names = [ s.decode() for s in hdf5_file['aux_data_names'][0,:] ]
            self.param_names = [ s.decode() for s in hdf5_file['label_names'][0,:] ]
            full_data = pd.DataFrame( hdf5_file['phy_data'] ).to_numpy()
            #full_data = pd.DataFrame( hdf5_file['data'] ).to_numpy()
            full_stats = pd.DataFrame( hdf5_file['aux_data'] ).to_numpy()
    ...
    def get_CQR_constant(self, preds, true, inner_quantile=0.95, asymmetric = True):
        ...
        for i in range(preds.shape[2]):
            if asymmetric:
                ...
                if lower_p < 0.:
                    self.logger.write_log('train', 'get_CQR_constant: lower_p >= 0.')
                    lower_p = 0.
                if upper_p > 1.:
                    self.logger.write_log('train', 'get_CQR_constant: upper_p <= 1.')
                    upper_p = 1.
